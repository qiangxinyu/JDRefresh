...
- (void)endRefresh
{
    if (self.isBegin) {
        ...
        [UIView animateWithDuration:.3 animations:^{
            self.imageView.center = CGPointMake(kScreenWidth, self.imageView.center.y);
...
- (void)startRefreshWithOffsetY:(CGFloat)currentOffsetY isHeader:(BOOL)isHeader
{
    if (!self.isBegin) {
        self.isBegin = YES;
        [UIView animateWithDuration:.3 animations:^{
            self.imageView.center = CGPointMake(kScreenWidth/5, self.imageView.center.y);
        }];
    }
    ...
    {
        if (currentOffsetY <=  (self.scrollView.contentSize.height -  self.scrollView.frame.size.height + 1)) {
            self.imageView.center = CGPointMake(0, self.imageView.center.y);
            self.isBegin = NO;
        }
    }
...
{
    if (!_logoImageView) {
        _logoImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 194/2.0, 70/2.0)];
        _logoImageView.image = [UIImage imageNamed:@"pull_to_refresh_text_jddj"];
        _logoImageView.center = CGPointMake(kScreenWidth/2, 64/2);
    }
...
{
    if (!_imageView) {
        _imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 63, 60.5)];
        NSMutableArray * array = [NSMutableArray array];
        for (int i = 0 ; i <  4 ; i ++) {
            [array addObject:[UIImage imageNamed:[NSString stringWithFormat:@"pull_to_refresh_people_%d.png",i]]];
        }
...
- (UIView *)backgroudView
{
    if (!_backgroudView) {
        ...
        [self addSubview:_backgroudView = view ];
